...
namespace WindowsFormsApp1
{
    public partial class FormHD : Form
    {
        ...
        private void buttonThem_Click(object sender, EventArgs e)
        {
            try
            {
                lenh = @"INSERT INTO hdnhap_xuat
                     (sohd, ngay, makh, sotientt, loaihd)
                        VALUES (@sohd,@ngay,@makh,@sotientt,@loaihd)";
                thuchien = new SqlCommand(lenh, ketnoi);
                thuchien.Parameters.Add("@sohd", SqlDbType.NChar).Value = textBoxSoHD.Text;
                thuchien.Parameters.Add("@ngay", SqlDbType.NChar).Value = textBoxNgayLap.Text;
                thuchien.Parameters.Add("@makh", SqlDbType.NChar).Value = comboBoxMaKH.Text;
                thuchien.Parameters.Add("@sotientt", SqlDbType.Float).Value = textBoxSoTien.Text;
                thuchien.Parameters.Add("@loaihd", SqlDbType.NVarChar).Value = comboBoxLoaiPh.Text;
                ketnoi.Open();
            ...
            catch (SqlException ex)
            {
                ...
                textBoxSoHD.Focus();
                //"Số hóa đơn \"" + textBoxSoHD.Text + "\" đã tồn tại"
            }
            finally
            {
                ketnoi.Close();
        ...
        {
            lenh = @"UPDATE hdnhap_xuat
                    SET       ngay = @ngay, makh = @makh, sotientt = @sotientt, loaihd = @loaihd
                    WHERE (sohd = @sohd)";
            thuchien = new SqlCommand(lenh, ketnoi);
